...
Pour la petite histoire, Gosset a travaillé pour une certaine brasserie irlandaise du nom de Guiness au début du 20^e^ siècle. C'est en étudiant la variabilité de sa bière d'un cru à l'autre que Gosset a découvert la façon dont la moyenne d'un échantillon se distribue. Il a pu dériver une formulation mathématique de cette distribution, la **distribution *t* de Student**, et à partir de là, nous verrons que de nombreuses applications en découlent. Nous pourrons, par exemple, dire si deux moyennes diffèrent **significativement** l'une de l'autre ou pas.

Mais au fait, pourquoi, cette distribution porte-t-elle le nom de "Student" ? Visionnez la vidéo suivante (malheureusement en anglais) pour le découvrir[^moyenne-1].

[^moyenne-1]: Vous pouvez activer les sous-titres en anglais via la barre de boutons en bas de la vidéo pour vous aider à comprendre l'histoire.
...
\end{equation}
```
A noter que $s^2$ est également appelée la **variance**[^moyenne-2].

[^moyenne-2]: L'équation proposée est, en fait, valable pour un échantillon, et est calculé comme tel par R à l'aide des fonctions `sd()` pour l'écart type ou `var()` pour la variance. Pour la population ou pour un échantillon de taille très grande, voire infinie, nous pourrions plutôt diviser par $n$ au lieu de $n - 1$, ... mais puisque $n$ est très grand, cela ne change pas grand chose au final.

En fait, ce qui nous intéresse, ce n'est pas vraiment la moyenne de l'échantillon, mais celle de la population que l'on notera $\mu$[^moyenne-3].

[^moyenne-3]: Notez que les lettres latines sont utilisées pour se référer aux variables et aux descripteurs statistiques telle que la moyenne pour l'échantillon, alors que les paramètres équivalents de la population, qui sont inconnus, sont représentés par des lettres grecques en statistiques.
...
-   Quelle est la probabilité que la moyenne d'un échantillon diffère de 0,5 unités de la vraie valeur ? Au lieu de considérer l'aire à gauche ou à droite, on considérera **une aire répartie symétriquement à moitié à gauche et à moitié à droite**. La réponse à la question est :
...
Étant donné l'importance que revet $\frac{s_x}{\sqrt{n}}$, nous appelerons cette quantité **erreur standard** de x et nous la noterons $SE_x$.
...
##### Conditions de validité de l'IC {.unnumbered}
...
En épluchant la littérature, nous réalisons que les crabes de la famille des Grapsidae dont *L. variegatus* fait partie ont systématiquement un arrière plus large chez la femelle lorsqu'un dimorphisme sexuel existe. Nous pouvons modifier nos hypothèses comme suit :
...
```{block, type='warning'}
Attention\ ! Les hypothèses se formulent différemment. Il s'agit de la différence des moyennes pour le test *t* indépendant et la moyenne des différences pour le test *t* apparié. Si le résultat du calcul en ce qui concerne la moyenne ne diffère pas, la distribution des valeurs est différente (variance, écart type, erreur standard, ...). 
```
...
Le test correspondant s'obtient dans la SciViews Box à partir du menu `.hm` pour `hypothesis tests: means` et dans le menu qui apparaît, on sélectionne le "snippet" `paired Student's t test`. Il s'agit toujours de la fonction `t.test()` mais cette fois-ci au lieu d'une formule, nous indiquons les deux variables suivant la notation `<data.frame>$<variable>`, et nous spécifions `paired = TRUE`. Fixons notre seuil $\alpha$ à 5% avant de faire le test, puis exécutons l'instruction suivante :
...
Nous rejetons $H_0$ car la valeur *P* est inférieure à $\alpha$. Nous en concluons que le crabe *L. variegatus* a une carapace significativement plus large à l'avant qu'à l'arrière (t test apparié = 25,3, ddl = 199, valeur *P* \<\< 10^-3^). Naturellement, les variantes unilatérales à gauche et à droite existent aussi pour ce test. Par contre, la question des variances égales ou non ne se pose pas dans ce cas[^moyenne-6]. Notez que le test se limite à indiquer que la valeur *P* \< 2,2 . 10^-16^. Effectivement, notre calcul donne 3.7 . 10^-64^.

[^moyenne-6]: Pouvez-vous expliquer pourquoi ? Réfléchissez, c'est assez évident.
...
Le calcul est le même que dans le cas du test *t* apparié, avec l'étape de soustraction `x1 - x2` en moins. Par exemple dans le jeu de données `sleep2` nous pouvons nous demander si le médicament `med1` change la durée du sommeil. Les hypothèses sont les suivantes :
...
La valeur *P* est supérieure à $\alpha$. Nous ne rejetons pas $H_0$. Nous ne pouvons pas dire que le médicament 1 change de manière significative la durée de sommeil chez les patients au seuil $\alpha$ de 5% (test t univarié = 1,33, ddl = 9, valeur *P* = 0,22). Notez également que dans le cas présent, l'intervalle de confiance [-0,53, 2,03] contient zéro.
...
```{r assign_A09Ia_cardiovascular, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("A09Ia_cardiovascular", part = NULL,
    url = "https://github.com/BioDataScience-Course/A09Ia_cardiovascular",
    course.ids = c(
      'S-BIOG-027' = !"A09Ia_{YY}M_cardiovascular", 
      'S-BIOG-921' = !"A09Ia_{YY}C_cardiovascular"),
    course.urls = c(
      'S-BIOG-027' = "https://classroom.github.com/a/UOQwKFmU", 
      'S-BIOG-921' = "https://classroom.github.com/a/..."),
    course.starts = c(
      'S-BIOG-027' = !"{W[27]+1} 15:00:00"),
    course.ends = c(
      'S-BIOG-027' = !"{W[29]+1} 23:59:59"),
    term = "Q2", level = 3,
    toc = "Maladies cardiovasculaires")
```
...
Pour les données relatives à `med1` de `sleep2`, cela donne[^moyenne-7] :

[^moyenne-7]: Dans le cas d'un test de Wilcoxon apparié, on a $H_0: mediane_x = mediane_y$.
...
-   Rejeter $H_0$ lorsque $H_0$ est vraie (erreur de première espèce $\alpha$)
-   Ne pas rejeter $H_0$ lorsque $H_0$ est fausse (erreur de seconde espèce $\beta$)
...
##### Pièges et astuces : barres d'erreurs {.unnumbered}
...
```{r assign_A09Ga_human_health_II, echo=FALSE, results='asis'}
if (exists("assignment2"))
  assignment2("A09Ga_human_health", part = "II",
...
if (exists("assignment2"))
  assignment2("A09Ga_urchin", part = "III",
    ...
    course.ends = c(
      'S-BIOG-027' = !"{W[37]+1} 23:59:59"),
    term = "Q2", level = 4, n = 2,
    toc = "Biométrie de l'oursin violet, part III")
```
